...
|Nutzerfreundlichkeit
a|
- *Erlernbarkeit*: Das System sollte verständlich und einfach bedienbar sein. Das lässt sich z.B. durch eindeutige Beschreibung von Eingaben mit Hilfe von Tooltips und/oder Labels realisieren.
...
- *Integrität*: Nicht-autorisierte Modifikationen sollten verhindert werden. Dafur kann die Spring Security (`@PreAuthorize` - annotation) verwendet werden.
