...
class PickUpControllerTests {
	...
	void pickupPost() throws Exception {
		...
		assertThat(itemCatalog.findByName("encrypted message").get().count()).isEqualTo(0);
	}
	...
	private MissMintOrder createOrder() {
		...
		MissMintOrder order = new MissMintOrder(userAccount, "Bob", now.minusMonths(1), service, orderItem);
